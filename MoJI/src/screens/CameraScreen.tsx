...
export function CameraScreen(): React.ReactElement {
  ...
  const serverUrl = config.aiServerUrl;
  ...
  // camera -> JNI -> C++ (MoJIFastStreaming)
  useEffect(() => {
  if (!isTransmitting || !isFocused) return
  ...
  const captureLoop = async () => {
    ...
    while (!isCancelled && isTransmitting) {
      try {
        if (camera.current && isFocused && isTransmitting) {
          ...
          if (blob.size < 1000) {
            console.warn('[warning] Blob too small, skipping...')
            continue
          }
          ...
          const result = moji.sendFrame(arrayBuffer, serverUrl)
          console.log(`Send to ${serverUrl}`)
            
        } else {
            console.warn('[warning] camera not ready')
        }
      } catch (error) {
          console.error('[failed] photo capture failed:', error)
  ...
  }, [isTransmitting, isFocused, effectiveFps, serverUrl])
  ...
  const getStatusColor = () => {
    if (!isTransmitting) return '#FF9800'
    if (stats.totalSent === 0) return '#888888'
    if (stats.successRate >= 90) return '#4CAF50'
    if (stats.successRate >= 70) return '#FFC107'
    return '#F44336'
  }
