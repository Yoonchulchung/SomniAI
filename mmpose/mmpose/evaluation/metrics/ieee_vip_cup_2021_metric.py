...
class IEEEVIP2021Metric(BaseMetric):
    default_prefix: Optional[str] = 'IEEEVIP2021'
    ...
    def __init__(self,
        ...
        if ann_file is not None:
            with get_local_path(ann_file) as local_path:
                self.ieee_2021 = json.load(open(local_path, 'r'))
        else:
            self.ieee_2021 = None
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if self.ieee_2021 is None:
            ...
            self.ieee_2021 = json.load(open(coco_json_path, 'r'))
        if self.gt_converter is not None:
            for id_, ann in self.ieee_2021.anns.items():
                self.ieee_2021.anns[id_] = transform_ann(
    ...
    def _do_python_keypoint_eval(self, outfile_prefix: str) -> list:
        ...
        """
        res_file = f'{outfile_prefix}.keypoints.json'
        coco_det = self.ieee_2021.loadRes(res_file)
        sigmas = self.dataset_meta['sigmas']
